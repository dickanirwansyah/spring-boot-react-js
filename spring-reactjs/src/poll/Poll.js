...
class Poll extends Component {
    ...
    isSelected = (choice) => {
        return this.props.poll.selectedChoice === choice.idchoie;
    }
        ...
        if(this.props.poll.selectedChoice || this.props.poll.expired) {
            ...
            this.props.poll.choices.forEach(choice => {
                pollChoices.push(<CompletedOrVotedPollChoice
                    key={choice.idchoie}
                    choice={choice}
                    isWinner={winningChoice && choice.idchoie === winningChoice.idchoie}
            ...
            this.props.poll.choices.forEach(choice => {
                pollChoices.push(<Radio className="poll-choice-radio" key={choice.idchoie} value={choice.idchoie}>{choice.text}</Radio>)
            })
        }
        return (
            <div className="poll-content">
                <div className="poll-header">
                    <div className="poll-creator-info">
                        <Link className="creator-link" to={`/users/${this.props.poll.userSummary.username}`}>
                            <Avatar className="poll-creator-avatar"
                                style={{ backgroundColor: getAvatarColor(this.props.poll.userSummary.name)}} >
                                {this.props.poll.userSummary.name[0].toUpperCase()}
                            </Avatar>
                            <span className="poll-creator-name">
                                {this.props.poll.userSummary.name}
                            </span>
                            <span className="poll-creator-username">
                                @{this.props.poll.userSummary.username}
                            </span>
